...
### READ DATA ###
dat <- read.csv('extension.csv')
all <- read.csv('data_083116.csv')
all$sex1<-factor(all$sex1)
...
try <- 0.35747 + BMI*-0.03535 + baseROM*0.0595 +sex*0.10227
...
all2 <- subset(all1, is.na(knee_arom_extension)==F & time_elapsed>0 & knee_arom_extension<11)
all3 <- all2[c("patient_id", "knee_arom_extension", "time_elapsed")]


mintime<-min(all3$time_elapsed)
maxtime<-max(all3$time_elapsed)
 
plm<-gamlss(knee_arom_extension~cs(time_elapsed^0.5),
            sigma.formula = ~cs(time_elapsed),
            data=na.omit(all3), family=NO)


all4<-subset(all3, is.na(knee_arom_extension)==F & time_elapsed<121)


# extract model parameters
xgrid <- c(2, seq(5, 120, 5))
xnew <- data.frame(patient_id = NA,
                   knee_arom_extension = NA, 
                   time_elapsed = xgrid)
ref <- predictAll(plm, 
...
ggplot(data = all4, aes(x = time_elapsed, y = knee_arom_extension)) +       
  ...
  labs(x="Time Following Surgery (Days)",y="Knee extension (Degrees)") +
theme(panel.grid.minor = element_line(colour="gray", size=0.5)) +
  scale_y_continuous(limits = c(-20,10), minor_breaks = seq(-20 , 10, 2), breaks = seq(-20, 10, 10)) +
